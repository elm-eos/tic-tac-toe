...
namespace tic_tac_toe {
  ...
  void apply_create(const create& c) {
    ...
    invite invite_to_create(c.challenger, c.host);
    Invites::store(invite_to_create, c.challenger);
  }
  ...
  void apply_close(const close& c) {
    ...
    // invite invite_to_close;
    // bool invite_exists = Invites::get(m.host, invite_to_close, m.challenger);
    // assert(invite_exists == true, "invite doesn't exist!");
    // Invites::remove(invite_to_close, invite_to_close.challenger);
  }
